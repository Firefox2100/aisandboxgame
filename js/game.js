...
你可以通过以下方式获取世界卡：
- 点击顶部的「**游戏模式 / 设计模式**」切换到 **设计模式**，与 AI 一起从零创建你的专属世界
...
function _buildUpdateChangelogHtml(entry) {
  ...
  const items = (entry.changes || [])
    .map(c => `<li>${escapeHTML(String(c).replace(/^[\s•·.]+/, ''))}</li>`)
    .join('');
...
function ensureGameModeSeedMessage() {
  ...
  // 防护：设计模式下 chatHistory 可能是 designChatHistory 的别名，
  // 此时绝不能 push 游戏开场白进去（详见司机案例：游戏开场词污染 designChatHistory）
  if (typeof isDesignMode !== 'undefined' && isDesignMode) {
    console.warn(
      '[ensureGameModeSeedMessage] 跳过：当前处于设计模式，避免污染 designChatHistory'
    );
    return { needsApiKeyHint: false };
  }
...
const HEADER_TOOLBAR_COLLAPSED_KEY = 'header-toolbar-collapsed';
const headerToolbarMobileMQ =
  typeof window !== 'undefined' && typeof window.matchMedia === 'function'
    ? window.matchMedia('(max-width: 768px)')
    : null;

function applyHeaderToolbarCollapsed(collapsed) {
  document.body.classList.toggle('header-toolbar-collapsed', collapsed);
  const toolbar = document.querySelector('.header-toolbar');
  if (toolbar) toolbar.setAttribute('aria-hidden', collapsed ? 'true' : 'false');
}

function setupTitleTips() {
  const trigger = document.getElementById('title-tips-trigger');
  if (!trigger || trigger.dataset.tipsBound === '1') return;

  // 启动时恢复折叠状态（仅移动端）
  if (headerToolbarMobileMQ && headerToolbarMobileMQ.matches) {
    try {
      if (localStorage.getItem(HEADER_TOOLBAR_COLLAPSED_KEY) === '1') {
        applyHeaderToolbarCollapsed(true);
      }
    } catch (_) {}
  }

  // 切回桌面端时清除折叠状态，避免桌面端被卡住
  if (headerToolbarMobileMQ) {
    const handleMqChange = e => {
      if (!e.matches) applyHeaderToolbarCollapsed(false);
    };
    if (typeof headerToolbarMobileMQ.addEventListener === 'function') {
      headerToolbarMobileMQ.addEventListener('change', handleMqChange);
    } else if (typeof headerToolbarMobileMQ.addListener === 'function') {
      headerToolbarMobileMQ.addListener(handleMqChange);
    }
  }

  function handleTipsClick() {
    // 移动端：点击标题切换顶部工具栏收起/展开
    if (headerToolbarMobileMQ && headerToolbarMobileMQ.matches) {
      const next = !document.body.classList.contains('header-toolbar-collapsed');
      applyHeaderToolbarCollapsed(next);
      try {
        localStorage.setItem(HEADER_TOOLBAR_COLLAPSED_KEY, next ? '1' : '0');
      } catch (_) {}
      return;
    }
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (!launcherVisible) {
    if (shouldOpenSaveManagerOnBoot && typeof openSaveManager === 'function') {
      openSaveManager(
        bootPreferredWorldCardId
          ? { preferredWorldCardId: bootPreferredWorldCardId, source: 'boot-resume' }
          : { source: 'boot-resume' }
      );
    } else {
  ...
  if (modeToggle) {
    modeToggle.addEventListener('click', async (event) => {
      ...
      if (
        ...
        showToast(
          isEn
            ? 'Design Mode requires an API key. Opening Settings…'
            : '设计模式需要先配置 API Key，已为你打开设置…'
        );
      ...
      // 必须在栏状态切换之前执行：CSS 默认隐藏规则依赖 #main-layout.design-mode-active
      const mainLayout = document.getElementById('main-layout');
      if (mainLayout) mainLayout.classList.toggle('design-mode-active', isDesign);

      // 模式切换时的输入栏状态：
      // - 设计模式：moveBarToBottom 搬回底部并清 inline-as-choice/bar-fallback/bar-hidden；
      //   设计模式 CSS 无隐藏规则，自然可见。再补一次 placeholder 重置（moveBarToBottom 不动 placeholder）
      // - 游戏模式：先搬回底部 + 清类，再尝试 relocate 到最新 D，失败则兜底底部
      // 兜底：先清 .bar-hidden，确保 refreshBarForGameMode 漏调（如脚本未加载）时
      // 输入栏也不会卡在隐藏态——CSS 翻转后默认即显示。
      const _inputAreaForToggle = document.querySelector('.chat-input-area');
      if (_inputAreaForToggle) _inputAreaForToggle.classList.remove('bar-hidden');
      if (isDesign) {
        window.moveBarToBottom?.();
        const _textbox = document.querySelector('.chat-input-area .chat-input-textbox');
        if (_textbox) {
          _textbox.setAttribute(
            'placeholder',
      ...
      // 设计模式：隐藏游戏模式 tab 容器（剧情总结/主角/角色档案），显示左侧 design header
      ...
      if (!isDesign) {
        ...
        // 重置 tab 到"对话"
        if (designHeader) {
          const tabs = designHeader.querySelectorAll('.tab');
      ...
      if (isDesign) {
        ...
        // 返回游戏模式：保存设计历史到 localStorage，恢复游戏历史
