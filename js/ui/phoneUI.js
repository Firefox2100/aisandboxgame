...
class PhoneUI {
  ...
  init() {
    ...
    if (phoneBtn) {
      phoneBtn.addEventListener('click', () => this.open());
    }
...
function handleSmsUnreadUpdate({ count }) {
  ...
  // 更新顶部手机按键红点
  const phoneBtnBadge = document.getElementById('phone-btn-badge');
  if (phoneBtnBadge) {
    if (count > 0) {
      phoneBtnBadge.textContent = badgeText;
      phoneBtnBadge.classList.remove('hidden');
    } else {
      phoneBtnBadge.classList.add('hidden');
    }
  }
}
