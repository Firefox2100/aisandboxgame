...
class _AIServiceNpcOocMixin {
  ...
  async _runNpcReactionCalls(adapter, messages, _systemContext) {
    ...
    results.forEach((result, i) => {
      ...
      if (result.status === 'fulfilled' && result.value) {
        ...
        console.log(`[NPC Decision] ${npc.name || npc.id}: ${decisionTag}`);
      } else {
        const reason = result.status === 'rejected' ? result.reason?.message : 'empty response';
        console.warn(`[NPC Decision] ${npc.name || npc.id} 决策失败: ${reason}`);
      }
    });
  ...
  async _callSingleNpcReaction(adapter, npc, context) {
    ...
    // Part 6: 角色档案
    const npcJson = JSON.stringify(npc, null, 2);
    systemParts.push({ text: `## 你的角色档案\n\n${npcJson}`, cacheable: false });

    // Part 7: 其他角色的当前状态（上一轮决策结果）
    const otherNpcText = this._buildOtherNpcStatusText(context.reactionHistory, npc.id);
    if (otherNpcText) {
      systemParts.push({ text: otherNpcText, cacheable: false });
    }

    // Part 8: 你自己的历史决策
    const historyText = this._buildNpcReactionHistoryText(context.reactionHistory, npc.id);
    if (historyText) {
    ...
    try {
      ...
      // 解析结构化 JSON 决策
      const decision = this._parseNpcDecisionJson(text, npc.name || npc.id);
      return {
  ...
  async _callOocNormalizer(adapter, candidates, ctx = {}) {
    ...
    const temperature = this.getModuleTemperature('ooc_normalizer', 0.5, AI_REQUEST_SCOPED);
