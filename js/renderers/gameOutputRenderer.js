...
const gameOutputRenderer = {
  ...
  renderChoices(choices) {
    ...
    for (const choice of choices) {
      ...
      // 第二行:描述 + cost(如果有描述或cost才显示)
      if (choice.detail_text || choice.cost_hint) {
...
let _inputAreaOriginalParent = null;
let _inputAreaOriginalNextSibling = null;

function _cacheInputOriginalParent() {
  if (_inputAreaOriginalParent) return;
  const inputArea = document.querySelector('.chat-input-area');
  if (!inputArea) return;
  _inputAreaOriginalParent = inputArea.parentNode;
  _inputAreaOriginalNextSibling = inputArea.nextSibling;
}
...
function _collapseChoicesBlock(gameChoicesEl) {
  ...
  const inputArea = document.querySelector('.chat-input-area');
  if (inputArea && gameChoicesEl.contains(inputArea)) {
    requestAnimationFrame(() => _collapseChoicesBlock(gameChoicesEl));
    return;
  }

  gameChoicesEl.querySelectorAll('.choice-input-host').forEach(h => h.remove());
...
function moveBarToBottom() {
  ...
  if (inputArea.parentNode !== _inputAreaOriginalParent) {
    if (
      _inputAreaOriginalNextSibling &&
      _inputAreaOriginalParent.contains(_inputAreaOriginalNextSibling)
    ) {
      _inputAreaOriginalParent.insertBefore(inputArea, _inputAreaOriginalNextSibling);
    } else {
      _inputAreaOriginalParent.appendChild(inputArea);
    }
  }
  document.querySelectorAll('.choice-input-host').forEach(h => h.remove());
  // 清掉定位 class：moveBarToBottom 语义是"完全还原"。只搬 DOM 不清 class
  // 会让 inputArea 在 fallback parent 里残留 inline-as-choice 样式（flex:1 +
  // transparent + 无边框），撑满容器剩余空间，呈现为大块"裸态"色块。
  inputArea.classList.remove('inline-as-choice', 'bar-fallback', 'bar-hidden');
}

function relocateInputIntoLatestChoices() {
  if (window.isDesignMode) return false;
  ...
  if (!latest.contains(inputArea)) {
    let host = latest.querySelector('.choice-input-host');
    if (!host) {
      host = document.createElement('div');
      host.className = 'choice-input-host';
      const letter = String.fromCharCode(
        'A'.charCodeAt(0) + latest.querySelectorAll('.choice-item').length
      );
      const customLabel = _i18n('choices.customLabel', '自定义');
      host.innerHTML = `
        <span class="choice-id">${letter}.</span>
        <span class="choice-type">[${customLabel}]</span>
        <div class="choice-input-slot"></div>
      `;
      latest.appendChild(host);
    }
    host.querySelector('.choice-input-slot').appendChild(inputArea);
  }
  inputArea.classList.remove('bar-fallback', 'bar-hidden');
  inputArea.classList.add('inline-as-choice');
  const textbox = inputArea.querySelector('.chat-input-textbox');
  if (textbox) {
...
function hideBar() {
  ...
  const inputArea = document.querySelector('.chat-input-area');
  if (!inputArea) return;
  // 显式加 .bar-hidden 才隐藏（CSS 翻转后默认显示）。
  // 这样任何 finalize 漏跑 / 异常路径都会留在"默认显示"状态，玩家始终能输入。
  inputArea.classList.remove('inline-as-choice');
  inputArea.classList.remove('bar-fallback');
  inputArea.classList.add('bar-hidden');
  if (typeof autoResizeTextarea === 'function') autoResizeTextarea();
}
...
function showFallbackBar() {
  ...
  if (!inputArea) return;
  moveBarToBottom();
  inputArea.classList.remove('inline-as-choice', 'bar-hidden');
  inputArea.classList.add('bar-fallback');
  const textbox = inputArea.querySelector('.chat-input-textbox');
  if (textbox) {
    textbox.setAttribute(
      'placeholder',
      _i18n('launcher.chatPlaceholder', '输入你的行动...')
    );
  }
  if (typeof autoResizeTextarea === 'function') autoResizeTextarea();
}

function refreshBarForGameMode() {
  if (window.isDesignMode) return;
  if (relocateInputIntoLatestChoices()) return;
  showFallbackBar();
}
...
function bindChoiceClickEvents() {
  document.querySelectorAll('.choice-item').forEach(item => {
    ...
    item.addEventListener('click', function () {
      ...
      const detailText = this.querySelector('.choice-detail')?.textContent || '';
      const costHint =
...
window.relocateInputIntoLatestChoices = relocateInputIntoLatestChoices;
window.moveBarToBottom = moveBarToBottom;
window.hideBar = hideBar;
window.showFallbackBar = showFallbackBar;
// 暴露给 chatCore：finalize 时输入栏一搬走就同步折叠 turn N-1 的 choices，
// 避免 _collapseChoicesBlock 内部 rAF 续帧造成「跳后回拉」的可见闪动
window._markStaleChoices = _markStaleChoices;
window.refreshBarForGameMode = refreshBarForGameMode;
...
function handleStatusFieldEdit(field) {
  ...
  // 设计模式下不触发游戏槽位 autoSave（走设计态自己的持久化）
  if (window.isDesignMode) return;
...
// 输入栏可见性 watchdog
// ============================================
// 兜底机制：处理 isSending 因极端路径（unhandled rejection / OOC 链异常 /
// 第三方钩子抛错）卡 true 不复位，导致输入栏永久 .bar-hidden 的死锁。
//
// 设计取舍：watchdog 只解锁 .bar-hidden、不复位 isSending。
// 理由：若 AI 调用真在流式中（5min+ 边界场景如长推理 / 慢网），强行复位
// isSending 会让下次 handleSendMessage 不被 guard 拦截，触发并发 turn
// → chatHistory 双写 / aiService 状态争抢 / 数据混乱。代价更高。
// 让步方案：用户至少能看到输入栏 UI（不再"无界面死锁"），但提交仍会被
// isSending guard 拦截，能感知到问题去刷新——比误杀活跃 streaming 安全。
//
// 5 分钟阈值远超单次 AI 调用合理时长（通常 10-180s），到此 hideBar
// 状态视为可疑；30 秒轮询，开销可忽略。
let _barWatchdogStuckSince = 0;
const _BAR_WATCHDOG_THRESHOLD_MS = 5 * 60 * 1000;
const _BAR_WATCHDOG_INTERVAL_MS = 30 * 1000;

function _runBarWatchdog() {
  if (window.isDesignMode || !window.isSending) {
    _barWatchdogStuckSince = 0;
    return;
  }
  const inputArea = document.querySelector('.chat-input-area');
  if (!inputArea || !inputArea.classList.contains('bar-hidden')) {
    _barWatchdogStuckSince = 0;
    return;
  }
  if (_barWatchdogStuckSince === 0) {
    _barWatchdogStuckSince = Date.now();
    return;
  }
  if (Date.now() - _barWatchdogStuckSince >= _BAR_WATCHDOG_THRESHOLD_MS) {
    console.warn('[bar-watchdog] isSending+bar-hidden >5min — unlocking bar (isSending preserved)');
    inputArea.classList.remove('bar-hidden');
    _barWatchdogStuckSince = 0;
  }
}

setInterval(_runBarWatchdog, _BAR_WATCHDOG_INTERVAL_MS);
