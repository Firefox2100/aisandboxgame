/**
 * design/ui.js
 * 预览面板 — 设计模式右侧 Card View / Code View 渲染
 *
...
class _DesignServiceUIMixin {
  ...
  _getViewMode() {
    const activeTab = document.querySelector('#design-chat-header .tab.is-active');
          ...
          subItems: events.map((e, i) => {
            ...
            return {
              ...
              preview: [e.location, e.content ? e.content.slice(0, 50) : '']
                ...
                .join(' — '),
              fullText,
              dragText: `[时间线 > 事件#${i + 1} (${e.time || ''} ${e.location || ''})] 请修改这个事件，具体要求：\n`,
              editTarget: 'timeline',
              editPath: `events[${i}]`,
              entityId: `事件#${i + 1}`,
            };
