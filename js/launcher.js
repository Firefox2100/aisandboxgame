  ...
  function getLauncherCopy() {
    ...
    return {
      ...
      preview: {
        ...
        modeGame: i18n?.t?.('launcher.modeGame') || (isEnglish ? 'Game Mode' : '游戏模式'),
        modeDesign: i18n?.t?.('launcher.modeDesign') || (isEnglish ? 'Design Mode' : '设计模式'),
      ...
      toast: {
        ...
        waitReplyBeforeDesignMode: isEnglish
          ? 'Wait for the current reply to finish before entering Design Mode.'
          : '请等待回复完成后再进入设计模式',
              ...
              if (typeof openSaveManager === 'function') {
                openSaveManager(
                  preferredWorldCardId
                    ? { preferredWorldCardId, source: 'launcher-continue' }
                    : { source: 'launcher-continue' }
                );
              }
            });
          });
