...
function _handleDiscardDraft() {
  ...
  const discard = () => {
    ...
    if (typeof isDesignMode !== 'undefined' && isDesignMode) {
      ...
      if (typeof refreshChatUI === 'function') refreshChatUI({ scrollMode: 'bottom' });
      if (typeof updateExecuteButtonState === 'function') updateExecuteButtonState('p1');
      if (window.worldCardInfoUI) window.worldCardInfoUI.refresh();
    }
    renderWorldCards();
  };
...
function _doActivateInDesignMode(id, cardName) {
  ...
  // 游戏模式下切换世界后：若历史为空则补种子消息，避免主界面空白
  if (
...
function _handleDeleteRequest(id) {
  ...
  if (modal) {
    const warningParts = [
      `确定要删除世界卡「${card.name}」吗？`,
      _worldCardUiText(
        `确定要删除世界卡「${card.name}」吗？`,
        `Delete the world card "${card.name}"?`
      ),
      _worldCardUiText(
        '该世界下的全部存档也会被删除。',
      ...
      isActive || isBound
        ...
        : '',
      _worldCardUiText('此操作不可撤销。', 'This action cannot be undone.'),
    ].filter(Boolean);
    modal.querySelector('#wc-delete-confirm-text').textContent = warningParts.join('');
...
function _doLoadCardIntoDesignMode(card) {
  ...
  if (typeof designChatHistory !== 'undefined') {
    ...
    // 过滤游戏开场词残留（INITIAL_GREETING_ZH/EN）——历史 bug 会让游戏模式
  ...
  // 6. 切换到设计模式
  if (typeof isDesignMode !== 'undefined' && isDesignMode) {
    // 已在设计模式：重新绑定 chatHistory 引用并刷新 UI
    if (typeof chatHistory !== 'undefined' && typeof designChatHistory !== 'undefined') {
      chatHistory = designChatHistory;
    }
    if (typeof refreshChatUI === 'function') refreshChatUI({ scrollMode: 'bottom' });
