...
/**
 * 序列化 NPC 档案的核心字段为 tool result 用对象。
 * processNpcPanel 通过 setTimeout 异步落库，execute 返回时 npcStore 还没新数据，
 * 所以入参必须是已组装好的 card-like 对象（pool 条目 / args / 现有 card），
 * 而不是依赖 npcStore.get(id)。
 */
const _NPC_CARD_TOOL_FIELDS = [
  'id', 'name', 'gender', 'origin', 'role', 'faction',
  'personality', 'appearance', 'clothing', 'msg_reply_tone',
  'cognitive_state', 'current_goal',
  'default_site', 'common_spots', 'routine',
];
function _serializeNpcCardFields(source) {
  if (!source || typeof source !== 'object') return null;
  const out = {};
  for (const key of _NPC_CARD_TOOL_FIELDS) {
    const v = source[key];
...
/**
 * 从世界卡 panel_npc schema 构建动态字段 properties
 * 剔除 trigger_type / id / name —— 这三个字段由各工具按用途单独处理。
...
function _registerNewNpc(register, dynamicFieldProps) {
  register('new_npc', {
    ...
    input_focus:
      'id 必须蛇形小写英文，不与预定义池任一 id 冲突；name 为角色显示名；其他字段优先标签式（如"沉稳/克制/隐忍"），关键字段（personality / appearance / role）需要细节时也可写一句话，单字段 ≤30 字。',
    expected_output:
      '返回 JSON {status:"created", card:{完整角色档案}}。card 即可直接用于叙事，无需再调读取工具。',
    parameters: {
      type: 'object',
      properties: {
        id: {
    ...
    execute(args) {
      ...
      npcStore.processNpcPanel([{ trigger_type: 'NEW', ...args }], turn, null);

      const card = _serializeNpcCardFields(args);
      return JSON.stringify({ status: 'created', card });
    },
    source: 'npc',
  });
}
...
function _registerUpdateNpc(register, dynamicFieldProps, spawnedIds) {
  ...
  // _buildDynamicFieldProps 已剔除 trigger_type/id/name；此处再剔除 gender/origin/birthday/age 等。
  ...
  register('update_npc', {
    ...
    signal: null,
    description:
      '更新已登场角色的运行时状态字段。一次调用处理一个角色。',
    when_to_call:
      '已存在角色的状态/外观/心理/关系/目标/语气/阵营发生明显变化时。',
    avoid_when:
      '角色未登场（id 不在 enum 内 schema 自动拒绝）；纯对话未改变角色状态；试图修改 id/name/gender/origin/birthday/age（schema 不暴露这些字段，无法修改）。错误示例：传 id="陌生 id" → schema enum 拒绝；传 gender="女" 想改性别 → schema 不暴露 gender 字段，根本无法填写。',
    input_focus:
      '只传 id + 实际变化的字段；字段值标签式 ≤3 词；变更进入待审批队列由玩家逐字段批准。',
    expected_output:
      '返回 JSON {status:"pending_review", id, requested_changes:[字段名...], current_card:{当前档案}}。变更需玩家批准，current_card 仍是审批前状态。',
    parameters: {
      type: 'object',
      properties: {
        id: {
    ...
    execute(args) {
      ...
      window._npcToolCalledThisTurn = true;
      const turn = (npcStore.currentTurn || 0) + 1;
      npcStore.processNpcPanel([{ trigger_type: 'UPDATE', ...args }], turn, null);

      const changedFields = Object.keys(args).filter(k => k !== 'id');
      const card = _serializeNpcCardFields(npcStore.get(id));
...
function _registerLoadPredefined(register, predefinedIds) {
  register('load_predefined_npc', {
    ...
    execute(args) {
      ...
      // 复刻 npcStore.processNpcPanel(NEW_PREDEFINED) 的字段组装逻辑（npcStore.js:502-521），
      ...
      const _fullState =
        typeof AnalyzerManager !== 'undefined'
          ? AnalyzerManager.getCharacterState(id, _currentTime)
          : null;
      const composed = {
        ...predefined,
        id: predefined.id || id,
        cognitive_state:
          _fullState?.cognitive_state || predefined.default_cognitive_state || predefined.cognitive_state,
      };
      if (_fullState) {
        for (const key of Object.keys(_fullState)) {
          if (key === 'cognitive_state') continue;
          if (_fullState[key] !== undefined && _fullState[key] !== null) {
            composed[key] = _fullState[key];
          }
        }
      }
      const card = _serializeNpcCardFields(composed);
      return JSON.stringify({ status: 'loaded', card });
    },
    source: 'npc',
  });
}
