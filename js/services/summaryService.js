...
class SummaryService {
  ...
  constructor() {
    ...
    this.summaryBtnBadge = document.getElementById('summary-btn-badge');
  ...
  // 当前活跃的 chatHistory 引用：设计模式 → designChatHistory（即 window.chatHistory），
  // 游戏模式 → 原 game chat。和 _collectAuditResult 的取法一致，避免在两种模式下查错历史
  ...
  flashErrorBorder() {
    ...
    // 移动端：显示红点
    if (this.summaryBtnBadge) {
      this.summaryBtnBadge.classList.remove('hidden');
    }
  }

  clearErrorBadge() {
    if (this.summaryBtnBadge) {
      this.summaryBtnBadge.classList.add('hidden');
    }
  }
  ...
  _collectAuditResult() {
    // 设计模式下 window.chatHistory 指向 designChatHistory，游戏 chat 在 _gameChatHistory；
