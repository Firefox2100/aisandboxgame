...
const RECOMMENDED_PHASE_MAP = Object.freeze({
  ...
  iter5_mutations:  Object.freeze({ model: 'deepseek-v4-flash', thinking: 'max'  }),
...
class AIService {
  constructor() {
    ...
    this.lastDesignPayload = null; // 保存最后一次设计模式请求的 payload
  ...
  _getDefaultTemperatureForModule(module, provider = null, options = {}) {
    const resolvedProvider = provider || this.getProviderForModule(module, options);
    if (module === 'react' && resolvedProvider === 'deepseek') {
      return 1.3;
    }
    return 1.0;
  }
  ...
  _buildInventorySystemText() {
    ...
    // 即使物品栏空（首回合）也要注入完整调用规则——否则 AI 看不到"何时该调"的强引导
    const lines = ['## 玩家物品栏（update_item 工具的当前状态）', ''];
