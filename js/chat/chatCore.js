...
function initChatSystem() {
  ...
  setupInputFocusKeyboardScroll();
}

// iOS 软键盘弹起时把 inline-as-choice 模式（D 自定义）的输入框滚入键盘上方。
// 详见 CLAUDE.md「主聊天区滚动条规矩」第 3 类白名单 input-focus-keyboard。
function setupInputFocusKeyboardScroll() {
  const vv = window.visualViewport;
  if (!vv) return;
  let prevHeight = vv.height;
  vv.addEventListener('resize', () => {
    const newHeight = vv.height;
    const keyboardOpening = newHeight < prevHeight - 100;
    prevHeight = newHeight;
    if (!keyboardOpening) return;
    if (document.activeElement !== chatInputTextbox) return;
    if (!chatMessagesArea?.contains(chatInputTextbox)) return;
    const rect = chatInputTextbox.getBoundingClientRect();
    const overflowBelow = rect.bottom + 16 - newHeight;
    if (overflowBelow <= 0) return;
    const target = chatMessagesArea.scrollTop + overflowBelow;
    // chat-scroll-allow: input-focus-keyboard
    chatMessagesArea.scrollTop = target;
  });
}

/**
 * 初始化卡片预览 → 输入框 拖拽注入功能
 * 当用户从设计模式卡片预览拖拽子项到输入框时，
...
function getUserLabel() {
  ...
  // 设计模式与 worldCardManager 的 active card 没有结构绑定，
  // 不附带世界卡名/存档名，只显示「你」。
 ...
 * 格式化主聊天 AI 标签
 */
function formatAiLabel(modelLabel, turn, thinkingLevel = null /* , uid 已迁出，改用 streamVisualizer.appendTurnUidBadge 悬浮显示 */) {
  const normalizedModel =
    typeof modelLabel === 'string' && modelLabel.trim() ? modelLabel.trim() : '模型';
  const normalizedTurn = Number.isFinite(turn) ? turn : '?';
  const isDeepSeek = inferProviderKeyFromModelLabel(normalizedModel) === 'deepseek';
  const thinkingPart = isDeepSeek ? formatThinkingMarker(thinkingLevel) : '';
...
/**
 * 解析 Design provider（用于设计模式头像 logo）
 * 优先级:
 * 1) msg.providerKey
 * 2) msg.modelLabel 推断
 ...
 */
/**
 * 设计模式自定义文本输入弹窗（替代 window.prompt）
 * @param {string} title  标题
...
async function handleSendMessage() {
  ...
  if (!window.isDesignMode) window.hideBar?.();
  isSending = true;

  // try/finally 必须从 isSending=true 之后立即开始包围——任何同步代码（OOC
  // 提取 / DOM 清空 / 埋点）抛错都得能跑 finally 复位 isSending + 显示输入栏，
  // 否则 isSending 卡 true + hideBar 已加 .bar-hidden = 永久死锁观感。
  try {
    // 游戏模式下抽取/剥离 OOC 候选（【...】或 [...]）。
    // 设计模式跳过，保留世界卡内容中的字面括号。
    ...
    if (!isDesignMode) {
      requestAnimationFrame(() => {
        ...
        const bar = document.querySelector('.chat-input-area');
        if (!bar) return;

        // 同帧 anchor 保留：finalize 时一切折叠/搬移都在一帧、一次画之前完成，避免
        // 浏览器画到「输入栏已搬走但 turn N-1 choices 还没折叠」的中间态产生可见跳动
        // 后再被拉回的闪动。Safari 18 原生 anchoring 在此场景失效；Chrome 此处 delta=0 无副作用。
        ...
        if (window.relocateInputIntoLatestChoices?.()) {
          // 输入栏已搬到 turn N。同步触发 turn N-1 的折叠（之前在 bindChoiceClickEvents
          // 时因输入栏在内被 _collapseChoicesBlock 延迟到下一帧；现在输入栏离开了，
          // 直接同步折叠避免延迟续帧造成可见闪动）
          window._markStaleChoices?.();
        } else {
          window.showFallbackBar?.();
        }

        // 同帧测量 + 调整 scrollTop（不再等 rAF）
...
async function handleDesignModeSendMessage(message, displayMessage) {
  ...
  try {
    if (!window.designService) {
      throw new Error('设计服务未初始化，请重新进入设计模式');
    }
    ...
    if (currentPhase === 'p1') {
      ...
      aiText = '当前阶段不支持对话操作。请等待自动生成完成，或重置设计模式。';
    }
    ...
    // 把设计模式 AI 错误送进 eventBus，让 Analytics 拿到结构化字段（phase/model/provider）
...
function _translateDesignErrorForUser(error) {
  ...
  // 设计模式缺少 API Key（结构化错误）
  if (safeError.code === 'DESIGN_API_KEY_MISSING') {
    friendlyMsg = '设计模式 API Key 未设置，请先在设置中填写并保存后重试。';
  }
  ...
  // 设计模式缺少 API Key（Gemini 常见原始错误兜底）
  if (
    !friendlyMsg &&
...
function _performChatUIRefresh(options = {}) {
  ...
  const restoreScrollPosition = () => {
    ...
    if (scrollMode === 'bottom') {
      chatMessagesArea.scrollTop = chatMessagesArea.scrollHeight; // chat-scroll-allow: history-replaced
      return;
    }
    chatMessagesArea.scrollTop = savedScrollTop; // chat-scroll-allow: rebuild-compensation
  };

  // 隐藏以防止闪烁
  chatMessagesArea.style.visibility = 'hidden';

  // 输入栏可能被 relocateInputIntoLatestChoices 移进 chatMessagesArea，
  // 下一行 innerHTML='' 会连带删除；先搬回原位避免节点永久丢失
  if (typeof window.moveBarToBottom === 'function') {
    window.moveBarToBottom();
  }
  ...
  const messageInfos = chatHistory.map((msg, originalIndex) => {
    ...
    if (msg.sender === 'user') {
      ...
      if (isDesignMode) {
        ...
        name = formatAiLabel(modelLabel, aiTurnCount, thinkingLevel);
  ...
  setTimeout(() => {
    ...
    // 存档/历史重建后，根据最新 choices-list 决定输入栏状态
    if (!isDesignMode && !isSending) {
      requestAnimationFrame(() => {
        if (window.isDesignMode) return;
        window.refreshBarForGameMode?.();
      });
    }
  }, 10);
}
