...
function _saveEditorPromptsToActiveWorldCard(greeting, initModule) {
  ...
  // Init 模块 UI 已移到设计模式；未传入时保留世界卡现有值，避免被清空
...
const SETTINGS_LOCALE_COPY = {
  'zh-CN': {
    ...
    custom: {
      ...
      help: {
        systemPrompt:
          '<p><strong>用途：</strong>在此写入的内容会追加到系统默认 Prompt 之后，作为额外指令影响 AI 的每一轮输出。可以理解为给 AI 的"常驻备忘录"。</p><p><strong>建议：</strong>适合用来调整 AI 的语言风格（如更诗意、更口语化）、添加主题限制（如禁止某类内容）、或强调特定行为准则。保持简洁清晰效果最好，避免与默认 Prompt 冲突。</p><p><strong>修改：</strong>在下方文本框中直接输入你的自定义指令。留空则不追加任何额外指令。例如：「回复时使用更加诗意的语言风格」「每次描写场景时加入天气和光影细节」。</p>',
        reset:
  ...
  en: {
    ...
    custom: {
      ...
      systemPrompt: {
        ...
        hint: 'Extra instructions appended after the default prompt.',
      ...
      help: {
        systemPrompt:
          '<p><strong>Purpose:</strong> Anything written here is appended after the default system prompt and affects every AI reply.</p><p><strong>Recommended:</strong> Use this for tone, constraints, or standing rules. Keep it short and clear.</p><p><strong>How to change:</strong> Type your extra instruction below. Leave it empty to disable it.</p>',
        reset:
...
const MODEL_PRICES = {
  // Gemini
  'gemini::gemini-3.1-flash-lite-preview': { in: 0.25, out: 1.5 },
  ...
  // Grok
  'grok::grok-4-1-fast-reasoning': { in: 0.2, out: 0.5 },
  // Anthropic
  'anthropic::claude-sonnet-4-6': { in: 3, out: 15 },
  'anthropic::claude-opus-4-6': { in: 5, out: 25 },
};

const MODULE_TEMPERATURE_MIN = 0;
const MODULE_TEMPERATURE_MAX = 2;
...
// 设计模式选了不稳定模型时显示警示。检测条件：provider 或 model id 含 'doubao'
...
function _getDefaultTemperatureForUIModule(uiModuleId, providerId) {
  const moduleId = _getConfigModuleIdByUI(uiModuleId);
  if (moduleId === 'react' && providerId === 'deepseek') return 1.3;
  return 1.0;
}
...
function _switchApiSettingsMode(mode) {
  ...
  _withStableSettingsAnchor(recommendedToggleRow, () => {
    ...
    if (mode === 'simple') {
      // 高级 → 常规：从 react 读取值填入"游戏模式"面板，从 design 读取值填入"设计模式"面板
      const reactProvider =
...
function setupSettingsUI() {
  ...
  // 加载自定义 System Prompt
  const customPromptTextarea = document.getElementById('custom-system-prompt');
  if (customPromptTextarea) {
    customPromptTextarea.value = config.customSystemPrompt || '';
  }
...
function saveSettings() {
  ...
  // 获取自定义 System Prompt
  const customPromptTextarea = document.getElementById('custom-system-prompt');
  const customSystemPrompt = customPromptTextarea?.value?.trim() || '';

  // 获取 Editor 开场白（Init 模块仅在设计模式编辑，这里保留现有值不覆盖）
  const greetingTextarea = document.getElementById('editor-greeting');
  ...
  // 收集 core_world_mechanics（通过设计模式修改）
