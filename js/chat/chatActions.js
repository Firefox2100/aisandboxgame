...
// 重新生成消息（常规模式会截断后续历史；设计模式 P2 走阶段重试，不截断历史）
async function regenerateMessage(msgIndex) {
  ...
  // 设计模式 P2：走统一重试入口（不能截断 chatHistory，也不能被 isSending 早锁阻塞）
  ...
  // 立刻刷新界面显示删除后的状态。refreshChatUI 内部会调 moveBarToBottom，
  // 后者已修复成"清 inline-as-choice / bar-fallback class"——
  // 配合 app.css:6309 的 :not(.inline-as-choice):not(.bar-fallback) → display:none，
  // 流式期间 input-area 自动隐藏。流式完成后由 finally 块 rAF 触发 relocate。
  ...
  try {
    ...
    // 与 handleSendMessage 末尾保持一致（chatCore.js:2054-2062）：
    // 流式完成后把输入栏挂回最新 turn 的 choices-list；找不到则回 fallback。
    // 缺这一步会让 input-area 永久停留在 _inputAreaOriginalParent 的"裸态"
    // （既无 .bar-fallback 也无 .inline-as-choice），表现为白色色块。
    if (!isDesignMode) {
      requestAnimationFrame(() => {
        if (window.isDesignMode) return;
        // 同帧 anchor 保留（同 chatCore.js handleSendMessage 末尾），兜 Safari
        ...
        if (window.relocateInputIntoLatestChoices?.()) {
          // 同步折叠 turn N-1（避免延迟续帧导致可见闪动），见 chatCore.js 同位置注释
...
function editMessage(msgIndex) {
  ...
  buttonContainer.innerHTML = `
        <button class="btn-primary btn-icon" data-action="edit-save-btn" type="button">保存</button>
        <button class="btn-secondary btn-icon" data-action="edit-cancel-btn" type="button">取消</button>
    `;
