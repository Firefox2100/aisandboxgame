...
const streamVisualizer = {
  ...
  _formatAiLabel(modelLabel, turnNumber, thinkingLevel = null /* , uid 已迁出，改用 appendTurnUidBadge 悬浮显示 */) {
    ...
    const isDeepSeek = this._inferProviderKeyFromModelLabel(normalizedModel) === 'deepseek';
  ...
  start(streaming = true) {
    ...
    const cancelBtn =
      '<button class="" data-action="stream-cancel-btn" type="button" aria-label="取消" title="取消">'
    ...
    const isDeepSeek = this._inferProviderKeyFromModelLabel(resolvedModelLabel) === 'deepseek';
    ...
    bubble.innerHTML = `
            <div class="chat-user-label">
                ${modelLabel} ${thinkingFragment}${indicator} ${cancelBtn}
            </div>
            ...
            <!-- Footer 占位 -->
            <div class="message-footer stream-slot" data-slot="footer">
                <div class="metrics-placeholder">
                    <div class="skeleton-metrics"></div>
                </div>
                <div class="message-actions skeleton-actions"></div>
            </div>
        `;
    ...
    document.querySelectorAll('#chat-container .status-field-value.status-editable').forEach(el => {
    ...
    // 绑定取消按钮（替代底部栏的 cancel-mode 发送按钮）
    const cancelBtnEl = bubble.querySelector('[data-action~="stream-cancel-btn"]');
    if (cancelBtnEl) {
      cancelBtnEl.addEventListener('click', (e) => {
        e.stopPropagation();
        if (cancelBtnEl.disabled) return;
        cancelBtnEl.disabled = true;
        if (window.aiService && typeof window.aiService.cancelRequest === 'function') {
          window.aiService.cancelRequest();
        }
      });
    }
  ...
  _doUpdate() {
    ...
    requestAnimationFrame(() => {
      ...
      // 隐藏骨架屏，显示内容；scroll 由浏览器原生 anchoring 维持。
      ...
      // 修复未闭合的 Markdown 语法
      const safeText = this._fixUnclosedMarkdown(displayText);

      // 渲染 Markdown（通过统一安全层）
      let htmlContent = '';
      if (window.htmlSecurity) {
        htmlContent = window.htmlSecurity.markdownToSafeHtml(safeText);
      } else {
        htmlContent = this._sanitizeHTML(safeText.replace(/\n/g, '<br>'));
      }

      narrativeEl.innerHTML = htmlContent;

      // 添加打字光标（使用捕获的引用）
      this._addCursorTo(narrativeEl);
  ...
  // ========================================
  _addCursor() {
    this._addCursorTo(this.state.narrativeElement);
  },

  // 添加打字光标到指定元素（供 RAF 回调使用捕获的引用）
  _addCursorTo(narrativeEl) {
    if (!narrativeEl) return;

    // 移除旧光标
    const oldCursor = narrativeEl.querySelector('.streaming-cursor');
    if (oldCursor) {
      oldCursor.remove();
    }
  ...
  finish(rawResponse, options = {}) {
    ...
    if (this.state.pendingText && this.state.narrativeElement) {
      ...
      if (displayText && displayText.length >= this.state.lastTextLength) {
        // 隐藏骨架屏；scroll 由浏览器原生 anchoring 维持。
        // 锁住 game-output 当前高度防止骨架→narrative 短暂塌缩闪动
        ...
        this.state.narrativeElement.innerHTML = htmlContent;
  ...
  _applyFinalContent(bubble, parsedJson, rawResponse, options, _isNonStreaming = false) {
    ...
    if (labelEl) {
      ...
      labelEl.textContent = this._formatAiLabel(modelLabel, turnNumber, thinkingLevel);
    ...
    if (hasSegments) {
      // 交错段落已由迭代事件渲染，清理光标和流式标记
      interleavedContainer.querySelectorAll('.streaming-cursor').forEach(c => c.remove());
    ...
    bubble.querySelector('[data-action~="stream-cancel-btn"]')?.remove();
    ...
    // （每个 -260px），是 finalize 时 viewport 上方最大量的内容缩水。同帧手动锚定
    // 兜底——Chrome 上其原生 anchoring 已经修好（这里 delta=0 无副作用），Safari 18
    // 原生 anchoring 在此场景下不可靠，靠这里手动锚。
    if (typeof window.bindChoiceClickEvents === 'function') {
      const cma = document.querySelector('.chat-messages-area');
      let anchor = null;
      let anchorOffset = 0;
      if (cma) {
        const cmaTop = cma.getBoundingClientRect().top;
        for (const child of cma.querySelectorAll(':scope > .chat-message.streaming-complete, :scope > .chat-message.user-message, :scope > .chat-fold-bar, :scope > .chat-expanded-group')) {
      ...
      if (anchor && cma && cma.contains(anchor)) {
        const newOffset = anchor.getBoundingClientRect().top - cma.getBoundingClientRect().top;
        const delta = newOffset - anchorOffset;
        if (delta !== 0) {
          cma.scrollTop += delta; // chat-scroll-allow: rebuild-compensation
        }
      }
    }
  ...
  _collapseReactTrace(container) {
    ...
    traceWrapper.querySelectorAll('.react-thinking-status--active').forEach(s => {
      ...
      if (startTime > 0) {
        ...
        s.textContent = template.replace('{n}', elapsed);
      }
    });
  ...
  _buildSummaryStrip(container, traceWrapper) {
    ...
    if (isStreaming) {
      ...
      statusSpan.textContent = i18n?.t?.('react.thinking') || '推理中';
      strip.appendChild(statusSpan);
      if (hasTools) {
        strip.appendChild(document.createTextNode(' '));
        strip.appendChild(this._cloneToolNamesSpan(lastToolsSpan));
      }
    } else {
      ...
      iterations.forEach(iter => {
        ...
        if (m) totalSec += parseInt(m[1], 10);
      });
      if (totalSec > 0) {
        const thoughtTpl = i18n?.t?.('react.thoughtFor') || '已推理 {n}s';
        strip.appendChild(document.createTextNode(' · ' + thoughtTpl.replace('{n}', totalSec)));
      }
      if (hasTools) {
        strip.appendChild(document.createTextNode(' · '));
        strip.appendChild(this._cloneToolNamesSpan(lastToolsSpan));
  ...
  abort() {
    if (this.state.bubbleElement) {
      ...
      this.state.bubbleElement.querySelector('[data-action~="stream-cancel-btn"]')?.remove();
...
if (window.eventBus && window.GameEvents) {
  ...
  eventBus.on(GameEvents.AI_REACT_TOOL_CALL, ({ iteration, calls }) => {
    ...
    if (statusSpan) {
      ...
      const i18n = typeof i18nService !== 'undefined' ? i18nService : null;
      const template = i18n?.t?.('react.thoughtFor') || '已推理 {n}s';
      statusSpan.textContent = template.replace('{n}', elapsed);
    }
  ...
  eventBus.on(GameEvents.AI_REACT_ITERATION_TEXT, ({ iteration, text }) => {
    ...
    const cursor = segment.querySelector('.streaming-cursor');
  ...
  function _renderTyperFrame() {
    ...
    segment.innerHTML = window.htmlSecurity
      ? window.htmlSecurity.markdownToSafeHtml(slice)
      : slice.replace(/\n/g, '<br>');
    streamVisualizer._addCursorTo(segment);
  ...
  eventBus.on(GameEvents.AI_NARRATIVE_STREAM, ({ iteration, text, isBlob }) => {
    ...
    _narrativeStreamRafId = requestAnimationFrame(() => {
      ...
      // 查找或创建该迭代的流式叙事段落（在 trace-wrapper 外）
      let segment = container.querySelector('.react-narrative-segment[data-streaming]');
      if (!segment) {
        streamVisualizer._onNarrativeStart(container);
        streamVisualizer._ensureTraceDivider(container);
      ...
      segment.innerHTML = window.htmlSecurity
        ? window.htmlSecurity.markdownToSafeHtml(pending.text)
  ...
  eventBus.on(GameEvents.AI_NARRATIVE_DISPLAY, ({ text }) => {
    if (!streamVisualizer.isStreaming()) return;
    if (!text) return;
    streamVisualizer._updateStreamProgressText('narrative');
    ...
    if (streamingSegment) {
      // 最终化：用完整文本替换，移除 cursor 和 streaming 标记
      streamingSegment.innerHTML = window.htmlSecurity
        ? window.htmlSecurity.markdownToSafeHtml(text)
        : text.replace(/\n/g, '<br>');
      const cursor = streamingSegment.querySelector('.streaming-cursor');
      if (cursor) cursor.remove();
      delete streamingSegment.dataset.streaming;
      streamVisualizer.state.narrativeElement = streamingSegment;
    } else {
      // 无流式段落（Gemini 等兜底）：创建完整段落
      streamVisualizer._onNarrativeStart(container);
      streamVisualizer._ensureTraceDivider(container);
