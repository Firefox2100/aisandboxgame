...
class DesignService {
  constructor() {
    ...
    this._allowOverwriteFromCardEdit = false; // 仅“在设计模式中编辑”入口允许覆盖
    this.pendingGameBootstrap = null; // 新世界应用成功后，切回游戏模式时的一次性引导标记
...
- 角色档案字段：确认角色需要哪些自定义追踪属性（如势力/境界/种族/职业等级），影响 extra_char_fields 输出`;
   ...
   *   - 游戏模式默认开场白（isDefaultOpeningGreeting）：来自游戏 chat 的污染，
   *     设计模式 chat 不应包含游戏开场词（详见司机案例）
  ...
  /**
   * 设计模式统一前置检查：内置 provider 必须配置 API Key
...
function initDesignService() {
  if (!window.designService) {
    ...
    if (window._pendingWorldCard) {
      ...
      // 向后兼容：从 localStorage 恢复设计模式草稿历史
